# Kanban Constraint Policy v1

This policy defines how PlanToDelivery maps provider results into Hermes Kanban task states. Hermes Kanban is the only executable authority for P2D/Javis V2.

## Canonical States

- `ready`: task can be assigned.
- `in_progress` / `running`: worker/provider is executing.
...
- Hermes Kanban card can be claimed.
...
|---|---|---:|
| `blocked=true` or blocking `result` | `blocked` | no |
| `review_required=true` | `review` | no |
| `status=completed` / `result=completed` and no review/block | `done` | yes |
| `status=partial` / `result=partial` with useful artifacts | `review` or follow-up card | no |
| `status=failed` / `result=failed` with retryable issue | `ready` or `blocked` after policy decision | no |
