...
- `KanbanStateStore(root)` is the JSON-backed fallback/export implementation under `project-state/kanban`: it writes task envelopes and result manifests as artifact/evidence files, keeps `kanban-state.json` synchronized for debug, fixtures, migration/export, and lightweight local dry-runs, and must not be treated as the execution state machine.
...
- **Hermes Kanban is the execution gate.** For real V2 orchestration, compile P2D capability/provider/gate work into Hermes Kanban tasks, links, claims, completes, blocks, reviews, runs, and events. Do not treat any project-local JSON/SQLite `status` field as the authority for whether work may start or finish. P2D storage is a semantic overlay and evidence/export layer; Hermes Kanban owns `todo/ready/running/review/blocked/done` lifecycle enforcement.
...
- `HermesKanbanBackend(project_root=..., board="plantodelivery", hermes_home=...)` is the minimal public-CLI adapter for real execution. It initializes/creates the board through `hermes kanban`, writes the same JSON task/result overlay as evidence, creates cards with `P2D_META`, maps local P2D task ids to Hermes `t_*` ids, and supports create/read/claim/complete/block/link without importing Hermes Agent internals. Enforcement rules now live here: provider results are rejected unless the Hermes card is currently `running`; `review_required=true` writes the result manifest, comments `P2D RESULT READY FOR REVIEW`, and blocks the card as a review gate instead of marking it done; `approve_review(task_id, evidence=[...])` requires non-empty review evidence, comments `P2D REVIEW APPROVED`, then completes the Hermes card; `audit_enforcement()` reports missing/invalid `P2D_META`, done cards without result manifests, and review-required done cards without approval evidence.
- `KanbanOrchestrator(project_root=..., providers_root=..., state_backend="json"|"hermes", state_store=...)` is the minimal contract facade: `dispatch_task(...)` selects a provider by capability and records JSON artifact overlay state for dry-runs/tests; with `state_backend="hermes"` it also creates the corresponding Hermes Kanban card and uses Hermes Kanban as the execution gate. `ingest_result(...)` or `ingest_result_path(...)` validates and persists provider result artifacts then updates overlay gate/card/event export state; `approve_review(task_id, evidence=[...])` advances a reviewed task inside the overlay only. `dispatch_next_ready_task()` is still a future automation layer; do not use it to bypass Hermes Kanban claim/complete/block transitions. Use `state_store=` only for explicit test/custom-store injection.
