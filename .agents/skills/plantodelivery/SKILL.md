...
- `KanbanOrchestrator(project_root=..., providers_root=..., state_backend="json"|"hermes", state_store=...)` is the minimal contract facade: `dispatch_task(...)` selects a provider by capability and records JSON artifact overlay state for dry-runs/tests; `ingest_result(...)` or `ingest_result_path(...)` validates and persists provider result artifacts then updates overlay gate/card/event export state; `approve_review(task_id, evidence=[...])` advances a reviewed task inside the overlay only. `dispatch_next_ready_task()` is intentionally disabled until the Hermes Kanban backend is implemented. Use `state_store=` only for explicit test/custom-store injection.
