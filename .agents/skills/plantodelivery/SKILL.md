...
- `JsonKanbanBoardAdapter(path)` persists the same card/event shape to a tiny JSON board snapshot as the first DB-backed state seam; swap this adapter for a real Kanban DB adapter without changing orchestrator/provider contracts.
