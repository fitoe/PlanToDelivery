...
- `KanbanStateStore(root)` is the JSON-backed fallback/export implementation under `project-state/kanban`: it writes task envelopes and result manifests as artifact/evidence files, and keeps `kanban-state.json` synchronized for debug, fixtures, and lightweight local dry-runs.
- `KanbanSQLiteStateStore(root, db_path=None)` is the preferred DB-backed canonical Kanban state source for V2 runtime work: SQLite owns `tasks`, `gates`, user-visible `cards`, append-only `events`, review records, artifact references, and provider recommendation fields. It preserves `suggested_gate_updates` / `next_recommended_task` as task DAG unlock inputs, exposes `find_next_ready_task()` for deterministic recovery of the next dispatchable DAG node after all `depends_on` tasks are completed, records idempotent `dependency_unlocked` events when review approval completes an upstream dependency, protects those unlock events with a stable `event_key` unique index for recovery/concurrency safety, auto-migrates legacy event tables that predate `event_key` without dropping historical events, and keeps JSON as an explicit export/evidence layer rather than the canonical source.
- Do not add board adapters for compatibility. If the state model changes, evolve `KanbanStateStore` / `KanbanSQLiteStateStore` and their contract tests directly.
...
- `KanbanOrchestrator(project_root=..., providers_root=..., state_backend="json"|"sqlite", state_store=...)` is the minimal end-to-end orchestration facade: `dispatch_task(...)` selects a provider by capability and records canonical Kanban state; `ingest_result(...)` or `ingest_result_path(...)` validates and persists provider result artifacts then updates canonical gate/card/event state; `dispatch_next_ready_task()` recovers the next `ready` DAG node from canonical state and dispatches it through the registry; `approve_review(task_id, evidence=[...])` advances a reviewed task from `review` to `completed` in canonical state. Use `state_backend="sqlite"` for the default recovery path: dispatch, result ingest, review approval, dependency unlock, orchestrator restart, and next-ready dispatch must all work through the rehydrated DB-backed store under `project-state/kanban`; pass `state_store=` only for explicit test/custom-store injection.
