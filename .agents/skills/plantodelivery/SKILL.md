...
- `KanbanSQLiteStateStore(root)` is the preferred DB-backed canonical Kanban state source for V2 runtime work: SQLite owns `tasks`, `gates`, user-visible `cards`, append-only `events`, review records, and artifact references; JSON remains an explicit export/evidence layer, not the canonical source.
