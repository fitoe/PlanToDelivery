...
- Review completion is where downstream children may be unlocked; when a task moves from `review` to `completed`, DB and JSON state stores must append `dependency_unlocked` events for newly dispatchable `ready` children whose `depends_on` are now all completed.
